...
public class SetArgument extends RankArgument {
    ...
    public String getDescription() {
        return "Sets player rank.";
    }

    @Override
    public String getUsage() {
        return "rank set [player] [rank] [reset-elo]";
    }
    ...
    private void execute(CommandSender sender, String[] args) {
        ...
        SPRank rank = getRankManager().getRank(rankId);
        if(rank == null) {
            MessageUtils.sendMessage(sender, "&cKhông thể tìm thấy cấp bậc có tên là &6" + rankId);
            return;
        }
        boolean resetElo = args.length == 3 && Boolean.parseBoolean(args[2]);
        SPRank oldRank = spPlayer.getRank();
        spPlayer.setRank(rank, resetElo);
        MessageUtils.sendMessage(sender, "&6" + playerName + ": &c" + oldRank.getDisplayName() + " &7» &a" + rank.getDisplayName());
    ...
    public List<String> tabComplete(CommandSender sender, String[] args) {
        ...
        if(args.length == 2)
            return getRankManager().getRanks().stream()
                    .map(SPRank::getId).filter(id -> id.startsWith(args[1]))
