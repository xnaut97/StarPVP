...
public class PromoteArgument extends RankArgument {
    public PromoteArgument(SPPlugin plugin) {
        super(plugin);
    }
    ...
    public String getDescription() {
        return "Promote player to next rank";
    }

    @Override
    public String getUsage() {
        return "rank promote [player] [reset-elo]";
    }
    ...
    private void execute(CommandSender sender, String[] args) {
        ...
        SPRank rank = oldRank.getNext();
        if(rank.isHighest()) {
            MessageUtils.sendMessage(sender, "&cNgười chơi này đã ở cấp bậc cao nhất rồi!");
            return;
        }
        boolean resetElo = args.length == 3 && Boolean.parseBoolean(args[2]);
        spPlayer.setRank(rank, resetElo);
        MessageUtils.sendMessage(sender, "&6" + playerName + ": " + "&a▲ " + rank.getDisplayName());
